...
public class GenericLLParser implements Parser {
    ...
    private AST parse(Rule rule) throws ParseException {
        ...
        for (int i = 0; i < rule.getRHS().size() & !rule.getRHS().isEmpty(); i++) {
            AST res = parse(rule.getRHS().get(i));
            if (res != null)
                result.addChild(res);
        }
        return result;
    ...
    private Map<NonTerm, Map<Term, Rule>> calcLL1Table() {
        ...
        for (Rule r : g.getRules()) {
            Map<Term, Rule> subResult = null;
            if (result.get(r.getLHS()) == null) {
                subResult = new HashMap<>();
            } else {
                subResult = result.get(r.getLHS());
            }

            for (Term t : firstp.get(r)) {
                subResult.put(t, r);
            }
            result.put(r.getLHS(), subResult);
        }
